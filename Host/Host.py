...
def Read_Data_From_Serial_Port(Command_Code):
    ...
    if(len(BL_ACK)):
        ...
        if(BL_ACK_Array[0] == 0xCD):
            ...
            print("   Preparing to receive (", int(
                Length_To_Follow), ") bytes from the bootloader")
...
def Process_CBL_GET_VER_CMD(Data_Len):
    ...
    print("   Bootloader Version   : ", _value_[
          1], ".", _value_[2], ".", _value_[3])
...
def Decode_CBL_Command(Command):
    ...
    if(Command == 1):
        ...
        CRC32_Value = Calculate_CRC32(
            BL_Host_Buffer, CBL_GET_RDP_STATUS_CMD_Len - 4)
        CRC32_Value = CRC32_Value & 0xFFFFFFFF
        BL_Host_Buffer[2] = Word_Value_To_Byte_Value(CRC32_Value, 1, 1)
        BL_Host_Buffer[3] = Word_Value_To_Byte_Value(CRC32_Value, 2, 1)
        BL_Host_Buffer[4] = Word_Value_To_Byte_Value(CRC32_Value, 3, 1)
        ...
        CRC32_Value = Calculate_CRC32(
            BL_Host_Buffer, CBL_GO_TO_ADDR_CMD_Len - 4)
        CRC32_Value = CRC32_Value & 0xFFFFFFFF
        ...
        print("Mass erase or page erase of the user flash command")
        ...
        SectorNumber = input(
            "\n   Please enter start page number(0-64)          : ")
        SectorNumber = int(SectorNumber, 10)
        if(SectorNumber != 0xFF):
            NumberOfSectors = int(
                input("\n   Please enter number of pages to erase (64 Max): "), 10)
        BL_Host_Buffer[2] = SectorNumber
        BL_Host_Buffer[3] = NumberOfSectors
        CRC32_Value = Calculate_CRC32(
            BL_Host_Buffer, CBL_FLASH_ERASE_CMD_Len - 4)
        CRC32_Value = CRC32_Value & 0xFFFFFFFF
        ...
        print("   Preparing writing a binary file with length (",
              File_Total_Len, ") Bytes")
        ...
        while(BinFileRemainingBytes):
            ...
            BL_Host_Buffer[2] = Word_Value_To_Byte_Value(
                BaseMemoryAddress, 1, 1)
            BL_Host_Buffer[3] = Word_Value_To_Byte_Value(
                BaseMemoryAddress, 2, 1)
            BL_Host_Buffer[4] = Word_Value_To_Byte_Value(
                BaseMemoryAddress, 3, 1)
            BL_Host_Buffer[5] = Word_Value_To_Byte_Value(
                BaseMemoryAddress, 4, 1)
            ...
            CRC32_Value = Calculate_CRC32(
                BL_Host_Buffer, CBL_MEM_WRITE_CMD_Len - 4)
            CRC32_Value = CRC32_Value & 0xFFFFFFFF
            BL_Host_Buffer[7 +
                           BinFileReadLength] = Word_Value_To_Byte_Value(CRC32_Value, 1, 1)
            BL_Host_Buffer[8 +
                           BinFileReadLength] = Word_Value_To_Byte_Value(CRC32_Value, 2, 1)
            BL_Host_Buffer[9 +
                           BinFileReadLength] = Word_Value_To_Byte_Value(CRC32_Value, 3, 1)
            BL_Host_Buffer[10 +
                           BinFileReadLength] = Word_Value_To_Byte_Value(CRC32_Value, 4, 1)
        ...
        if(Protection_level == 2):
            ...
            CRC32_Value = Calculate_CRC32(
                BL_Host_Buffer, CBL_CHANGE_ROP_Level_CMD_Len - 4)
            CRC32_Value = CRC32_Value & 0xFFFFFFFF
            ...
            for Data in BL_Host_Buffer[1: CBL_CHANGE_ROP_Level_CMD_Len]:
                Write_Data_To_Serial_Port(
                    Data, CBL_CHANGE_ROP_Level_CMD_Len - 1)
...
while True:
    print("\nSTM32F103C8T6 Custom BootLoader - Copyrights (c) Wx, Belkasy 2022")
    print("==============================")
